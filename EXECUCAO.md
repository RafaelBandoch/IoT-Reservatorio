...
senha reais do cluster ficam em quatro arquivos, todos fora do git:

| Lugar | Como preencher |
| --- | --- |
| `.env` | copie de `.env.example` e preencha `MQTT_REMOTE_HOST` / `MQTT_REMOTE_PASS` |
| `credenciais.h` | copie de `credenciais.exemplo.h`, ponha `MQTT_TLS 1` e as credenciais do cluster |
| `pgadmin/pgpass` | gere a partir do `.env` (ver "Como subir") |
| `painel-reservatorio.local.html` | cópia local do painel com `BROKER` e `SENHA` reais |

Os dois últimos da lista não têm como evitar: o navegador e o pgAdmin leem a
credencial do próprio arquivo. Se você compartilhar o projeto no Wokwi, o
conteúdo do `credenciais.h` vai junto — por isso use um cluster descartável e
rotacione a credencial no painel do HiveMQ quando terminar.
...
Para uma captura limpa, garanta que o `credenciais.h` aponta para um broker que
realmente responde — com um endereço inexistente o log enche de tentativas de
MQTT falhando:

```bash
cp credenciais.exemplo.h credenciais.h   # na primeira vez
./compilar.sh
```
